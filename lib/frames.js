...
// Frames scale linearly with lecture length and are deliberately never capped. The selected frames
// are the only evidence the prompt has for what appeared on screen, so a long lecture must not be
// silently thinned out to the same handful of captures as a short one. Local scouting still inspects
// many more frames than this; the budget is the number that survives ranking and reaches Gemini.
export const FRAMES_PER_MINUTE = 0.6;

export function frameBudget(durationSeconds) {
  const minutes = Math.max(0, Number(durationSeconds) || 0) / 60;
  // The floor of one keeps a very short clip from producing a transcript-only, frame-less run.
  return Math.max(1, Math.ceil(minutes * FRAMES_PER_MINUTE));
}
